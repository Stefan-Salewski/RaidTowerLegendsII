    ...
    if game_state == "menu":
        ...
        for chest in chests:
            ...
            screen.blit(chest_cost,
                        pygame.math.Vector2(chest.rect.centerx - main_camera.offset.x - chest_cost.get_width() / 2,
                                            chest.rect.centery - main_camera.offset.y))
            ...
            if player_entity.mask.overlap(chest.mask,
                                          collision_offset) and player_entity.money >= chest.cost and not chest.opened:
                chest.open_chest()
        ...
        screen.blit(level_counter,
                    ((screen.get_width() / 2) - level_counter.get_width() / 2, screen.get_height() - 1000))

        health_text = REGULAR_FONT.render(str(player_entity.max_health) + "/" + str(player_entity.health), True,
                                          colours[4])
        screen.blit(health_text, ((screen.get_width() / 2) - level_counter.get_width() / 2, screen.get_height() / 1.75))
