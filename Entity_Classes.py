...
# player class
class Player(Entity):
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT, bullet_img, bullets, game_state, invulnerability=False, health=100,
    ...
    def shoot_bullets(self, movement_vector, screen_instance, speed, damage, amount):
        ...
        for _ in range(amount):
            ...
            new_bullet = Bullet(damage, speed, offset_movement_vector, screen_instance, self.bullet_img,
...
class Bullet(Entity):
    def __init__(self, damage, speed, movement_vector, screen_instance, image, x, y):
        ...
        self.movement_vector = pygame.math.Vector2(
            movement_vector).normalize() * self.speed  # normalize and apply speed
...
class Enemy(Entity):
    ...
    def enemy_movement(self, screen_instance, player_ref, camera_offset):
        self.enemy_moving = pygame.math.Vector2(player_ref.rect.centerx - self.rect.centerx,
                                                (player_ref.rect.centery) - self.rect.centery)
        ...
        if self.enemy_moving.length() <= 1000 and self.enemy_moving.length() > 0:
            ...
            self.rect.move_ip(self.enemy_moving.x,
                              self.enemy_moving.y)  # moving it by whatever the new vectors coordinates are
